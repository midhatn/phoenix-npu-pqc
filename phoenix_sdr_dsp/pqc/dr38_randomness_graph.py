...
"""Computational Graph & Hardware Dispatch Orchestrator for Milestone DR38:
NIST SP 800-22 Randomness Statistical Battery & BSI AIS 31 Hardware Diagnostic on AMD Phoenix AIE2.
"""
...
BACKEND_LABEL = "dr38-randomness-battery:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr38_randomness_service.cc"
_PROGRAM: Any | None = None


class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR38 backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR38 physical silicon requires XRT device(0)") from exc
...
def _load_iron() -> tuple[Any, ...]:
    try:
        ...
        raise NativeBackendUnavailable(
            "DR38 requires MLIR-AIE/IRON, XRT, and an XRT-visible Phoenix NPU."
        ) from exc
    return (
        iron,
        CompileTime,
...
def _program() -> Any:
    ...
    def dr38_randomness_program(
        ...
        *,
        ...
        of_request = ObjectFifo(request_ty, name="dr38_request")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr38_descriptor")
        of_result = ObjectFifo(result_ty, name="dr38_result")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        service_fn = ExternalFunction(
            "dr38_randomness_service",
            source_file=str(kernel_path / "dr38_randomness_service.cc"),
    ...
    _PROGRAM = dr38_randomness_program
    return _PROGRAM


def _dispatch_dr38(desc_bytes: bytes, req_buf: bytes) -> Tuple[bytes, float]:
...
def run_dr38_randomness_on_aie2(
    op_mode: int,
    sample_bytes: bytes,
    block_size: int = 128,
    flags: int = 0,
    ...
    """[ON-TILE SILICON] Dispatches Randomness Statistical Battery operation to AMD Phoenix AIE2."""
    desc = pack_dr38_descriptor(
        op_mode=op_mode,
        sample_bytes_len=len(sample_bytes),
        block_size=block_size,
        flags=flags,
        seq_id=seq_id,
    )
    req = pack_dr38_request(
        sample_bytes=sample_bytes,
        seq_id=seq_id,
    )
    return _dispatch_dr38(desc, req)
