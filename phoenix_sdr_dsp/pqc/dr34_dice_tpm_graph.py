...
"""Computational Graph & Hardware Dispatch Orchestrator for Milestone DR34:
Hardware Root of Trust, TCG DICE / TPM Attestation & Enclave Security Boundaries on AMD Phoenix AIE2.
"""
...
BACKEND_LABEL = "dr34-dice-tpm:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr34_dice_tpm_service.cc"
_PROGRAM: Any | None = None


class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR34 backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR34 physical silicon requires XRT device(0)") from exc
...
def _load_iron() -> tuple[Any, ...]:
    try:
        ...
        raise NativeBackendUnavailable(
            "DR34 requires MLIR-AIE/IRON, XRT, and an XRT-visible Phoenix NPU."
        ) from exc
    return (
        iron,
        CompileTime,
...
def _program() -> Any:
    ...
    def dr34_dice_tpm_program(
        ...
        *,
        ...
        of_request = ObjectFifo(request_ty, name="dr34_request")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr34_descriptor")
        of_result = ObjectFifo(result_ty, name="dr34_result")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        service_fn = ExternalFunction(
            "dr34_dice_tpm_service",
            source_file=str(kernel_path / "dr34_dice_tpm_service.cc"),
    ...
    _PROGRAM = dr34_dice_tpm_program
    return _PROGRAM


def _dispatch_dr34(desc_bytes: bytes, req_buf: bytes) -> Tuple[bytes, float]:
    ...
    *_, XRTTensor = _load_iron()
    ...
    # In verify mode, quote failure returns non-zero status as expected verification outcome
    op_mode = struct.unpack_from("<I", raw_res, 4)[0]
    if op_mode != MODE_DICE_VERIFY_QUOTE and status != 0:
        raise RuntimeError(f"DR34 hardware error status: 0x{status:08X}")

    return raw_res, dt_ms


def run_dr34_dice_tpm_on_aie2(
    op_mode: int,
    pcr_index: int = 0,
    pcr_mask: int = 0x0F,
    ...
    req = pack_dr34_request(
        ...
        initial_pcr_bank=initial_pcr_bank,
        uds_key=uds_key,
        signature=signature,
        seq_id=seq_id,
        ak_pk=ak_pk,
