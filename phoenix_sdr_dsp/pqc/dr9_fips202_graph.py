...
"""DR9 Reusable NIST FIPS 202 NPU Service Graph on AMD Phoenix AIE2."""
...
from . import dr9_fips202_unified_abi as abi

BACKEND_LABEL = "dr9-fips202:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr9_fips202_service.cc"
...
class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR9 backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR9 physical silicon requires XRT device(0)") from exc


def get_kernel_artifact_info(repo_root: Path | None = None) -> dict[str, Any]:
    """Return verified path and SHA-256 digest of the DR9 AIE2 kernel source."""
...
def _load_iron() -> tuple[Any, ...]:
    ...
    try:
        ...
        raise NativeBackendUnavailable(
            "DR9 requires MLIR-AIE/IRON 1.4.1, XRT, and an XRT-visible Phoenix NPU."
        ) from exc
    return (
        iron,
        CompileTime,
...
def _program() -> Any:
    ...
    def dr9_fips202_program(
        ...
        *,
        ...
        of_request = ObjectFifo(request_ty, name="dr9_request")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr9_descriptor")
        of_result = ObjectFifo(result_ty, name="dr9_result")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        fips202_fn = ExternalFunction(
            "dr9_fips202_service",
            source_file=str(kernel_path / "dr9_fips202_service.cc"),
        ...
        worker = Worker(
            worker_body,
            fn_args=[of_request.cons(), of_descriptor.cons(), of_result.prod(), fips202_fn],
            stack_size=0x2000,
        )
    ...
    _PROGRAM = dr9_fips202_program
    return _PROGRAM

def run_fips202_service(func_name: str, msg: bytes, out_len: int | None = None, request_id: int = 1) -> bytes:
    """Execute FIPS 202 function on physical AMD Phoenix NPU."""
    *_, XRTTensor = _load_iron()
    ...
    default_out_lens = {
        ...
        "SHA3-384": 48,
        "SHA3-512": 64,
        "SHAKE128": 32 if out_len is None else out_len,
        "SHAKE256": 64 if out_len is None else out_len,
    }
    actual_out_len = default_out_lens[func_upper] if out_len is None else out_len

    req_bytes = abi.pack_dr9_request(msg)
    desc_bytes = abi.pack_dr9_descriptor(func_upper, len(msg), actual_out_len, request_id=request_id)
    ...
    req_id_out, status, out_len_out, digest, crc32 = abi.unpack_dr9_result(raw_output)

    if status != 0:
        raise RuntimeError(f"DR9 FIPS 202 service returned error status {status}")
