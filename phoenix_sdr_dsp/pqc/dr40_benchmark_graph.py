...
"""Computational Graph & Hardware Dispatch Orchestrator for Milestone DR40:
Reproducible High-Throughput Hardware Benchmark Protocol & Profiling Battery on AMD Phoenix AIE2.
"""
...
from .dr40_benchmark_abi import (
    MAGIC_HEADER,
...
BACKEND_LABEL = "dr40-hardware-benchmark:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr40_benchmark_service.cc"
_PROGRAM: Any | None = None


class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR40 backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR40 physical silicon requires XRT device(0)") from exc
...
def _load_iron() -> tuple[Any, ...]:
    try:
        ...
        raise NativeBackendUnavailable(
            "DR40 requires MLIR-AIE/IRON, XRT, and an XRT-visible Phoenix NPU."
        ) from exc
    return (
        iron,
        CompileTime,
...
def _program() -> Any:
    ...
    def dr40_benchmark_program(
        ...
        *,
        ...
        of_request = ObjectFifo(request_ty, name="dr40_request")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr40_descriptor")
        of_result = ObjectFifo(result_ty, name="dr40_result")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        service_fn = ExternalFunction(
            "dr40_benchmark_service",
            source_file=str(kernel_path / "dr40_benchmark_service.cc"),
    ...
    _PROGRAM = dr40_benchmark_program
    return _PROGRAM


def _dispatch_dr40(desc_bytes: bytes, req_buf: bytes) -> Tuple[bytes, float]:
...
def run_dr40_benchmark_on_aie2(
    op_mode: int,
    batch_size: int = 8,
    warmup_iters: int = 2,
    ...
    """[ON-TILE SILICON] Dispatches high-throughput benchmark workload to AMD Phoenix AIE2."""
    ...
    if request_bytes is None:
        req_buf = bytes(REQUEST_BUFFER_SIZE)
    elif len(request_bytes) < REQUEST_BUFFER_SIZE:
        req_buf = request_bytes + bytes(REQUEST_BUFFER_SIZE - len(request_bytes))
    else:
        req_buf = request_bytes[:REQUEST_BUFFER_SIZE]

    return _dispatch_dr40(desc, req_buf)
