...
# DR15: Complete NIST FIPS 204 ML-DSA-87 Signature Verification Graph.
# 100% On-Device ML-DSA-87 Verify on AMD Phoenix NPU (AIE2 / XDNA1).
import hashlib
import os
import sys
...
BACKEND_LABEL = "dr15-mldsa87-verify:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr15_mldsa87_verify_w2_fin.cc"

# Sizes
REQ_BYTES = 7300          # pk(2592) + mu(64) + sig(4627) + pad
DESCRIPTOR_BYTES = 16
TOKEN0_BYTES = 19000
TOKEN1_BYTES = 72
...
class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR15 Verify backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR15 physical silicon requires XRT device(0)") from exc


def get_kernel_artifact_info(repo_root: Path | None = None) -> dict[str, Any]:
    """Return verified path and SHA-256 digest of the DR15 Verify AIE2 kernel source."""
...
def _build_dr15_verify_program():
    ...
    def dr15_verify_pipeline(
        ...
        *,
        ...
        of_req = ObjectFifo(req_ty, name="dr15_ver_req")
        of_desc = ObjectFifo(descriptor_ty, name="dr15_ver_desc")
        of_t0 = ObjectFifo(token0_ty, name="dr15_ver_t0")
        of_t1 = ObjectFifo(token1_ty, name="dr15_ver_t1")
        of_res = ObjectFifo(result_ty, name="dr15_ver_res")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        inc_dirs = [cxx_header_path(), str(kernel_path)]

        w0_fn = ExternalFunction("dr15_mldsa87_verify_w0_init", source_file=str(kernel_path / "dr15_mldsa87_verify_w0_init.cc"), arg_types=[req_ty, descriptor_ty, token0_ty], include_dirs=inc_dirs)
        w1_fn = ExternalFunction("dr15_mldsa87_verify_w1_matrix", source_file=str(kernel_path / "dr15_mldsa87_verify_w1_matrix.cc"), arg_types=[token0_ty, token1_ty], include_dirs=inc_dirs)
        w2_fn = ExternalFunction("dr15_mldsa87_verify_w2_fin", source_file=str(kernel_path / "dr15_mldsa87_verify_w2_fin.cc"), arg_types=[token1_ty, result_ty], include_dirs=inc_dirs)
    ...
    return dr15_verify_pipeline

def _program():
    global _CACHED_PROGRAM
    if _CACHED_PROGRAM is None:
        _CACHED_PROGRAM = _build_dr15_verify_program()
...
def run_mldsa87_verify(
    ...
    """Execute 100% On-Device ML-DSA-87 Verification on physical Phoenix NPU.
    ...
    *_, XRTTensor = _load_iron()
    ...
    req_buf[:2592] = pk[:2592]
    req_buf[2592 : 2592 + min(len(mu), 64)] = mu[:64]
    req_buf[2656 : 2656 + min(len(sig), 4627)] = sig[:4627]
