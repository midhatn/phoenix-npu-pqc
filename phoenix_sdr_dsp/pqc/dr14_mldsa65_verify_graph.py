...
# DR14: Complete NIST FIPS 204 ML-DSA-65 Verification Graph.
# 100% On-Device Device-Resident ML-DSA-65 Verify on AMD Phoenix NPU (AIE2 / XDNA1).
import hashlib
import os
import sys
...
BACKEND_LABEL = "dr14-mldsa65-verify:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr14_mldsa65_verify_w2_fin.cc"
...
class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR14 Verify backend is unavailable or failed closed."""
...
def get_kernel_artifact_info(repo_root: Path | None = None) -> dict[str, Any]:
    """Return verified path and SHA-256 digest of the DR14 Verify AIE2 kernel source."""
...
def _build_dr14_verify_program():
    ...
    def dr14_verify_pipeline(
        ...
        *,
        ...
        of_req = ObjectFifo(req_ty, name="dr14_ver_req")
        of_desc = ObjectFifo(descriptor_ty, name="dr14_ver_desc")
        of_t0 = ObjectFifo(token0_ty, name="dr14_ver_t0")
        of_t1 = ObjectFifo(token1_ty, name="dr14_ver_t1")
        of_res = ObjectFifo(result_ty, name="dr14_ver_res")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        inc_dirs = [cxx_header_path(), str(kernel_path)]

        w0_fn = ExternalFunction("dr14_mldsa65_verify_w0_init", source_file=str(kernel_path / "dr14_mldsa65_verify_w0_init.cc"), arg_types=[req_ty, descriptor_ty, token0_ty], include_dirs=inc_dirs)
        w1_fn = ExternalFunction("dr14_mldsa65_verify_w1_matrix", source_file=str(kernel_path / "dr14_mldsa65_verify_w1_matrix.cc"), arg_types=[token0_ty, token1_ty], include_dirs=inc_dirs)
        w2_fn = ExternalFunction("dr14_mldsa65_verify_w2_fin", source_file=str(kernel_path / "dr14_mldsa65_verify_w2_fin.cc"), arg_types=[token1_ty, result_ty], include_dirs=inc_dirs)
    ...
    return dr14_verify_pipeline

def _program():
    global _CACHED_PROGRAM
    if _CACHED_PROGRAM is None:
        _CACHED_PROGRAM = _build_dr14_verify_program()
...
def run_mldsa65_verify(
    ...
    """Execute 100% On-Device ML-DSA-65 Verify on physical Phoenix NPU.
    Returns: True if signature is valid, False otherwise.
    """
    if len(pk) != 1952 or len(sig) != 3309:
        return False
    ...
    *_, XRTTensor = _load_iron()
    ...
    req_buf[:1952] = pk[:1952]
    req_buf[1952 : 1952 + min(len(mu), 64)] = mu[:64]
    req_buf[2016 : 2016 + min(len(sig), 3309)] = sig[:3309]
