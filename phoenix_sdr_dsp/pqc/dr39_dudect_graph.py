...
"""Computational Graph & Hardware Dispatch Orchestrator for Milestone DR39:
dudect Side-Channel Timing & TVLA Constant-Time Diagnostic Engine on AMD Phoenix AIE2.
"""
...
BACKEND_LABEL = "dr39-dudect-tvla:silicon"
KERNEL_REL_PATH = "phoenix_sdr_dsp/pqc/kernels/dr39_dudect_service.cc"
_PROGRAM: Any | None = None


class NativeBackendUnavailable(RuntimeError):
    """The native IRON/XRT DR39 backend is unavailable or failed closed."""
...
def require_hardware_runtime() -> None:
    ...
    try:
        ...
        raise NativeBackendUnavailable("DR39 physical silicon requires XRT device(0)") from exc
...
def _load_iron() -> tuple[Any, ...]:
    try:
        ...
        raise NativeBackendUnavailable(
            "DR39 requires MLIR-AIE/IRON, XRT, and an XRT-visible Phoenix NPU."
        ) from exc
    return (
        iron,
        CompileTime,
...
def _program() -> Any:
    ...
    def dr39_dudect_program(
        ...
        *,
        ...
        of_request = ObjectFifo(request_ty, name="dr39_request")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr39_descriptor")
        of_result = ObjectFifo(result_ty, name="dr39_result")

        kernel_path = Path(__file__).resolve().parent / "kernels"
        service_fn = ExternalFunction(
            "dr39_dudect_service",
            source_file=str(kernel_path / "dr39_dudect_service.cc"),
    ...
    _PROGRAM = dr39_dudect_program
    return _PROGRAM


def _dispatch_dr39(desc_bytes: bytes, req_buf: bytes) -> Tuple[bytes, float]:
...
def run_dr39_dudect_on_aie2(
    ...
    """[ON-TILE SILICON] Dispatches dudect Side-Channel Timing Diagnostic operation to AMD Phoenix AIE2."""
    ...
    req = pack_dr39_request(
        class0_seed=class0_seed,
        class1_seed=class1_seed,
        payload=payload,
        seq_id=seq_id,
    )
    return _dispatch_dr39(desc, req)
