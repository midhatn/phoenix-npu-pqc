...
"""DR8 ML-KEM-1024 Decaps dataflow graph on AMD Phoenix NPU (Ryzen 7040 / 8040 AIE2)."""
from pathlib import Path
from typing import Any
import numpy as np

BACKEND_LABEL = "dr8-mlkem1024-decaps:silicon"
_PROGRAM: Any | None = None

REQ_BYTES = 4736 # dk[3168] || c[1568]
DESCRIPTOR_BYTES = 16
DERIV_TOKEN_BYTES = 3760
NOISE_TOKEN_BYTES = 8336
ROW0_TOKEN_BYTES = 8176
ROW1_TOKEN_BYTES = 8016
ROW2_TOKEN_BYTES = 7856
ROW3_TOKEN_BYTES = 7696
...
def _program() -> Any:
    ...
    @iron.jit
    def dr8_mlkem1024_decaps_program(
        req_in: In,
        descriptor_in: In,
        result_out: Out,
        *,
        ...
        row3_token_slots: CompileTime[int],
        ...
        row3_token_ty = np.ndarray[(row3_token_slots,), np.dtype[element_type]]
        result_ty = np.ndarray[(result_slots,), np.dtype[element_type]]

        of_req = ObjectFifo(req_ty, name="dr8_dec1024_req")
        of_descriptor = ObjectFifo(descriptor_ty, name="dr8_dec1024_descriptor")
        of_deriv = ObjectFifo(deriv_token_ty, name="dr8_dec1024_deriv")
        of_noise = ObjectFifo(noise_token_ty, name="dr8_dec1024_noise")
        of_row0 = ObjectFifo(row0_token_ty, name="dr8_dec1024_row0")
        of_row1 = ObjectFifo(row1_token_ty, name="dr8_dec1024_row1")
        of_row2 = ObjectFifo(row2_token_ty, name="dr8_dec1024_row2")
        of_row3 = ObjectFifo(row3_token_ty, name="dr8_dec1024_row3")
        of_result = ObjectFifo(result_ty, name="dr8_dec1024_result")
        ...
        fn_decrypt = external("dr8_mlkem1024_decaps_decrypt", "dr8_mlkem1024_decaps_decrypt.cc", [req_ty, descriptor_ty, deriv_token_ty])
        fn_noise = external("dr8_mlkem1024_decaps_noise", "dr8_mlkem1024_decaps_noise.cc", [deriv_token_ty, noise_token_ty])
        fn_row0 = external("dr8_mlkem1024_decaps_row0", "dr8_mlkem1024_decaps_row0.cc", [noise_token_ty, row0_token_ty])
        fn_row1 = external("dr8_mlkem1024_decaps_row1", "dr8_mlkem1024_decaps_row1.cc", [row0_token_ty, row1_token_ty])
        fn_row2 = external("dr8_mlkem1024_decaps_row2", "dr8_mlkem1024_decaps_row2.cc", [row1_token_ty, row2_token_ty])
        fn_row3 = external("dr8_mlkem1024_decaps_row3", "dr8_mlkem1024_decaps_row3.cc", [row2_token_ty, row3_token_ty])
        fn_final = external("dr8_mlkem1024_decaps_finalize", "dr8_mlkem1024_decaps_finalize.cc", [row3_token_ty, result_ty])
        ...
        workers = [
            ...
            Worker(unary_body, fn_args=[of_row2.cons(), of_row3.prod(), fn_row3], stack_size=0x2000),
            Worker(unary_body, fn_args=[of_row3.cons(), of_result.prod(), fn_final], stack_size=0x2000),
        ]
    ...
    _PROGRAM = dr8_mlkem1024_decaps_program
    return _PROGRAM

def run_mlkem1024_decaps(dk: bytes, c: bytes, req_id: int = 1):
    ...
    *_, XRTTensor = _load_iron()
    ...
    try:
        _program()(
            ...
            row3_token_slots=ROW3_TOKEN_BYTES,
    ...
    if status != 0:
        raise RuntimeError(f"ML-KEM-1024 Decaps failed on silicon with status {status}")
