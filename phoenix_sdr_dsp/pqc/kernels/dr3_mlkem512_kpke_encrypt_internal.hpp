...
// Private DR3 token layouts and operation-local helpers for ML-KEM-512 K-PKE.Encrypt.
#pragma once
...
#define DR3_DISABLE_UNROLL _Pragma("clang loop unroll(disable)")
#else
#define DR3_DISABLE_UNROLL
#endif

namespace phoenix_sdr_dsp::pqc::dr3 {

constexpr uint32_t kN = 256u, kQ = 3329u, kRate128 = 168u, kRate256 = 136u;
constexpr uint32_t kBlockCap = 5u;
constexpr uint32_t kOk = 0u, kLimitExceeded = 1u, kBadDescriptor = 2u, kBadToken = 3u;

// Token Sizes
constexpr uint32_t kNoiseTokenBytes = 3632u;
constexpr uint32_t kCol0TokenBytes = 4656u;
constexpr uint32_t kU0TokenBytes = 3440u;
constexpr uint32_t kCol1TokenBytes = 4464u;
constexpr uint32_t kResultBytes = 800u;

// Noise Token Offsets (3632 B)
constexpr uint32_t kNoiseHeaderBytes = 16u;
constexpr uint32_t kRhoOffset = 16u;       // 32 B
constexpr uint32_t kR0Offset = 48u;         // 512 B
constexpr uint32_t kR1Offset = 560u;        // 512 B
constexpr uint32_t kE1_0Offset = 1072u;     // 512 B
constexpr uint32_t kE1_1Offset = 1584u;     // 512 B
constexpr uint32_t kE2MuOffset = 2096u;     // 512 B
constexpr uint32_t kT0Offset = 2608u;       // 512 B
constexpr uint32_t kT1Offset = 3120u;       // 512 B

// Col0 Token Offsets (4656 B)
constexpr uint32_t kA00Offset = 3632u;      // 512 B
constexpr uint32_t kA10Offset = 4144u;      // 512 B

// U0 Token Offsets (3440 B)
constexpr uint32_t kU0HeaderBytes = 16u;
constexpr uint32_t kU0RhoOffset = 16u;      // 32 B
constexpr uint32_t kU0C1_0Offset = 48u;     // 320 B
constexpr uint32_t kU0R0Offset = 368u;      // 512 B
constexpr uint32_t kU0R1Offset = 880u;      // 512 B
constexpr uint32_t kU0E1_1Offset = 1392u;   // 512 B
constexpr uint32_t kU0E2MuOffset = 1904u;   // 512 B
constexpr uint32_t kU0T0Offset = 2416u;     // 512 B
constexpr uint32_t kU0T1Offset = 2928u;     // 512 B

// Col1 Token Offsets (4464 B)
constexpr uint32_t kA01Offset = 3440u;      // 512 B
constexpr uint32_t kA11Offset = 3952u;      // 512 B

// Result Offsets (800 B)
constexpr uint32_t kResultHeaderBytes = 20u;
constexpr uint32_t kResultCiphertextOffset = 20u; // 768 B

// FIPS 203 kZetas in 32-bit array (prevents AIE2 lda.u16 half-word indexing bug)
...
static inline bool valid_descriptor(const uint8_t d[16]) {
  return d[0] == 1 && d[1] == 0x31 && d[2] == 0x52 && d[3] == 0 &&
         d[4] == 2 && d[5] == 3 && d[6] == 2 && d[7] == kBlockCap &&
...
static inline uint32_t bit_at(const uint8_t *buf, uint32_t bit) {
  return (buf[bit >> 3] >> (bit & 7)) & 1u;
}

// Decode 384 bytes into 256 12-bit coefficients (512 bytes output)
__attribute__((noinline)) static void decode_d12(const uint8_t in[384], uint8_t out[512]) {
  DR3_DISABLE_UNROLL
  for (uint32_t i = 0; i < kN / 2; ++i) {
    uint32_t b0 = in[3 * i + 0];
    uint32_t b1 = in[3 * i + 1];
    uint32_t b2 = in[3 * i + 2];
    uint32_t d0 = (b0 | (b1 << 8)) & 0x0FFF;
    uint32_t d1 = ((b1 >> 4) | (b2 << 4)) & 0x0FFF;
    store_pair_word(out, i, d0, d1);
  }
}
...
// In-place Cooley-Tukey NTT with static stage unrolling
...
// In-place Gentleman-Sande INTT with static stage unrolling
...
// Expand one A[row, col] via rejection sampling
...
// SHAKE256 PRF (192 B for CBD3, 128 B for CBD2)
...
// Pointwise multiplication in NTT domain with BaseMul
...
// Accumulate acc = p0 * r0 + p1 * r1 in NTT domain
...
__attribute__((noinline)) static void compress10_encode(const uint32_t *u, uint8_t *out) {
  encode_16_coeffs(u + 0 * 16, out + 0 * 20);
  encode_16_coeffs(u + 1 * 16, out + 1 * 20);
  encode_16_coeffs(u + 2 * 16, out + 2 * 20);
  encode_16_coeffs(u + 3 * 16, out + 3 * 20);
  encode_16_coeffs(u + 4 * 16, out + 4 * 20);
  encode_16_coeffs(u + 5 * 16, out + 5 * 20);
  encode_16_coeffs(u + 6 * 16, out + 6 * 20);
  encode_16_coeffs(u + 7 * 16, out + 7 * 20);
  encode_16_coeffs(u + 8 * 16, out + 8 * 20);
  encode_16_coeffs(u + 9 * 16, out + 9 * 20);
  encode_16_coeffs(u + 10 * 16, out + 10 * 20);
  encode_16_coeffs(u + 11 * 16, out + 11 * 20);
  encode_16_coeffs(u + 12 * 16, out + 12 * 20);
  encode_16_coeffs(u + 13 * 16, out + 13 * 20);
  encode_16_coeffs(u + 14 * 16, out + 14 * 20);
  encode_16_coeffs(u + 15 * 16, out + 15 * 20);
}

static inline void encode_8_coeffs_4bit(const uint32_t v[8], uint8_t out[4]) {
  const uint32_t c0 = compress4_coeff(v[0]);
  const uint32_t c1 = compress4_coeff(v[1]);
  const uint32_t c2 = compress4_coeff(v[2]);
  const uint32_t c3 = compress4_coeff(v[3]);
  const uint32_t c4 = compress4_coeff(v[4]);
  const uint32_t c5 = compress4_coeff(v[5]);
  const uint32_t c6 = compress4_coeff(v[6]);
  const uint32_t c7 = compress4_coeff(v[7]);

  uint32_t *out_word = reinterpret_cast<uint32_t *>(out);
  *out_word = c0 | (c1 << 4) | (c2 << 8) | (c3 << 12) |
              (c4 << 16) | (c5 << 20) | (c6 << 24) | (c7 << 28);
}

__attribute__((noinline)) static void compress4_encode(const uint32_t *v, uint8_t *out) {
  for (uint32_t grp = 0; grp < 4; ++grp) {
    encode_8_coeffs_4bit(v + (8 * grp + 0) * 8, out + (8 * grp + 0) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 1) * 8, out + (8 * grp + 1) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 2) * 8, out + (8 * grp + 2) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 3) * 8, out + (8 * grp + 3) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 4) * 8, out + (8 * grp + 4) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 5) * 8, out + (8 * grp + 5) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 6) * 8, out + (8 * grp + 6) * 4);
    encode_8_coeffs_4bit(v + (8 * grp + 7) * 8, out + (8 * grp + 7) * 4);
  }
}

constexpr uint32_t crc32_byte(uint32_t b) {
  uint32_t crc = b;
...
static inline uint32_t compute_crc32(const uint8_t *data, uint32_t length) {
  ...
  for (uint32_t i = 0; i < length; ++i) {
    ...
    uint32_t table_val = crc32_byte(lookup);
    crc = (crc >> 8) ^ table_val;
  }
