...
extern "C" void dr15_mldsa87_verify_w0_init(
  ...
  const uint8_t *z_bytes = req_in + 2720;
  const uint8_t *h_bytes = req_in + 7200;
  ...
  DR11_DISABLE_UNROLL
  for (uint32_t i = 0; i < 64; ++i) out_token[100 + i] = c_tilde[i];

  bool valid = true;

  // 1. Decode & norm-check z (7 polys -> out_token[164..7331])
  int32_t poly[256];
  int32_t *z_hat = reinterpret_cast<int32_t *>(out_token + 164);
  ...
  // 2. Decode hints (83 B -> out_token[7332..9379])
  uint8_t h[8][256];
  if (!decode_hints87_and_check(h_bytes, h)) valid = false;
  uint8_t *h_dst = out_token + 7332;
  ...
  // 3. Decode t1 * 2^d -> NTT(t1 * 2^d) (8 polys -> out_token[9380..17571])
  int32_t *t1_hat = reinterpret_cast<int32_t *>(out_token + 9380);
  for (uint32_t r = 0; r < 8; ++r) {
  ...
  // 4. SampleInBall87(c_tilde) -> NTT(c) -> out_token[17572..18595]
  int32_t *c_hat = reinterpret_cast<int32_t *>(out_token + 17572);
  sample_in_ball87(c_tilde, poly);
  ntt_kernel(poly);
  DR11_DISABLE_UNROLL
  for (uint32_t c = 0; c < 256; ++c) c_hat[c] = poly[c];

  out_token[18596] = valid ? 1 : 0;
