...
// Private DR7 token layouts and operation-local helpers for ML-KEM-512 ML-KEM.Decaps.
#pragma once
...
#define DR7_DISABLE_UNROLL _Pragma("clang loop unroll(disable)")
#else
#define DR7_DISABLE_UNROLL
#endif

namespace phoenix_sdr_dsp::pqc::dr7 {
...
// Token Sizes
constexpr uint32_t kDerivationTokenBytes = 1968u;
constexpr uint32_t kNoiseTokenBytes = 4464u;
constexpr uint32_t kCol0TokenBytes = 5488u;
constexpr uint32_t kU0TokenBytes = 4272u;
constexpr uint32_t kCol1TokenBytes = 5296u;
constexpr uint32_t kResultBytes = 52u;

// Derivation Token Offsets (1968 B)
// Header: 16 B
constexpr uint32_t kDerivMPrimeOffset = 16u;   // 32 B
constexpr uint32_t kDerivHEkOffset = 48u;      // 32 B
constexpr uint32_t kDerivZOffset = 80u;        // 32 B
constexpr uint32_t kDerivRhoOffset = 112u;     // 32 B
constexpr uint32_t kDerivCOffset = 144u;       // 768 B
constexpr uint32_t kDerivT0Offset = 912u;      // 512 B
constexpr uint32_t kDerivT1Offset = 1424u;     // 512 B

// Noise Token Offsets (4464 B)
// Header: 16 B
constexpr uint32_t kNoiseKBarPrimeOffset = 16u; // 32 B
constexpr uint32_t kNoiseZOffset = 48u;         // 32 B
constexpr uint32_t kNoiseRhoOffset = 80u;       // 32 B
constexpr uint32_t kNoiseCOffset = 112u;        // 768 B
constexpr uint32_t kNoiseR0Offset = 880u;       // 512 B
constexpr uint32_t kNoiseR1Offset = 1392u;      // 512 B
constexpr uint32_t kNoiseE1_0Offset = 1904u;    // 512 B
constexpr uint32_t kNoiseE1_1Offset = 2416u;    // 512 B
constexpr uint32_t kNoiseE2MuOffset = 2928u;    // 512 B
constexpr uint32_t kNoiseT0Offset = 3440u;      // 512 B
constexpr uint32_t kNoiseT1Offset = 3952u;      // 512 B

// Col0 Token Offsets (5488 B)
constexpr uint32_t kA00Offset = 4464u;          // 512 B
constexpr uint32_t kA10Offset = 4976u;          // 512 B

// U0 Token Offsets (4272 B)
// Header: 16 B
constexpr uint32_t kU0KBarPrimeOffset = 16u;    // 32 B
constexpr uint32_t kU0ZOffset = 48u;            // 32 B
constexpr uint32_t kU0RhoOffset = 80u;          // 32 B
constexpr uint32_t kU0COffset = 112u;           // 768 B
constexpr uint32_t kU0C1_0Offset = 880u;        // 320 B
constexpr uint32_t kU0R0Offset = 1200u;         // 512 B
constexpr uint32_t kU0R1Offset = 1712u;         // 512 B
constexpr uint32_t kU0E1_1Offset = 2224u;       // 512 B
constexpr uint32_t kU0E2MuOffset = 2736u;       // 512 B
constexpr uint32_t kU0T0Offset = 3248u;         // 512 B
constexpr uint32_t kU0T1Offset = 3760u;         // 512 B

// Col1 Token Offsets (5296 B)
constexpr uint32_t kA01Offset = 4272u;          // 512 B
constexpr uint32_t kA11Offset = 4784u;          // 512 B

// Result Offsets (52 B)
constexpr uint32_t kResultHeaderBytes = 20u;
constexpr uint32_t kResultKeyOffset = 20u;      // 32 B
...
static inline bool valid_descriptor(const uint8_t descriptor[16]) {
  return descriptor[0] == 1u && descriptor[1] == 0x71u &&
...
// SHAKE256 for 800 bytes (z[32] || c[768] -> 32 bytes) with 32-bit word XORs
__attribute__((noinline)) static void shake256_800(const uint8_t z[32], const uint8_t c[768], uint8_t out[32]) {
  alignas(8) uint8_t state[200];
  clear_bytes(state, sizeof(state));
  ...
  // Blocks 1..4: c[104 + (b-1)*136 .. 104 + b*136] (34 words each)
  DR7_DISABLE_UNROLL
  ...
  // Block 5: remaining 120 bytes (30 words) of c + padding
  const uint32_t rem_w_off = 26 + 4 * 34; // 162 words
  DR7_DISABLE_UNROLL
  for (uint32_t i = 0; i < 30; ++i) state_w[i] ^= c_w[rem_w_off + i];
  state[120] ^= 0x1f; state[135] ^= 0x80;
  phoenix_sdr_dsp::pqc::dr1::keccak_f1600(state);
...
// Decode & decompress d10: 320 bytes -> 256 uint32 in Z_q
...
// Decode & decompress d4: 128 bytes -> 256 uint32 in Z_q
static inline void decode_decompress_d4(const uint8_t in[128], uint32_t out[kN]) {
  DR7_DISABLE_UNROLL
  for (uint32_t i = 0; i < 128; ++i) {
    const uint32_t c0 = in[i] & 0x0fu;
    const uint32_t c1 = (in[i] >> 4) & 0x0fu;
    out[2 * i + 0] = (c0 * 3329u + 8u) >> 4;
    out[2 * i + 1] = (c1 * 3329u + 8u) >> 4;
  }
}

// Compress1: 256 coefficients in Z_q -> 32 bytes message
static inline void compress1(const uint32_t w[kN], uint8_t out[32]) {
  clear_bytes(out, 32);
  DR7_DISABLE_UNROLL
  for (uint32_t i = 0; i < kN; ++i) {
    const uint32_t val = w[i];
    if (val >= 833u && val <= 2496u) {
      out[i >> 3] |= (1u << (i & 7u));
    }
  }
}
