...
// W1 receives Noise Token (3152 B) and computes t0 = A[0,0]*s0 + A[0,1]*s1 + A[0,2]*s2 + e0
// Output Token:
// [0..15]: Header
// [16..47]: rho (32)
// [48..79]: z (32)
...
extern "C" void dr8_mlkem768_keygen_row0(
  ...
  const uint8_t *e0 = in_token + 1616;

  alignas(4) uint8_t a00[512], a01[512], a02[512];
  ...
  uint32_t t0[kN];
  ntt_multiply_accumulate_3(a00, s0, a01, s1, a02, s2, t0);
  ...
  for (uint32_t pair = 0; pair < kN / 2; ++pair) {
    store_pair_word(out_token + 1616, pair, t0[2 * pair], t0[2 * pair + 1]);
  }
