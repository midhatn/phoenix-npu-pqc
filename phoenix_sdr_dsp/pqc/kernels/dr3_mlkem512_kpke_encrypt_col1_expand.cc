...
extern "C" void dr3_mlkem512_kpke_encrypt_col1_expand(
  ...
  // Copy u0 token prefix (3440 B)
  copy_words(col1_token, u0_token, kU0TokenBytes);

  const uint8_t *rho = u0_token + kU0RhoOffset;

  // A^T[1, 0] = SampleNTT(rho, 1, 0)
  if (!sample_matrix_store(rho, 1, 0, col1_token + kA01Offset)) {
    store_le32(col1_token + 4, kLimitExceeded);
    return;
  }

  // A^T[1, 1] = SampleNTT(rho, 1, 1)
  if (!sample_matrix_store(rho, 1, 1, col1_token + kA11Offset)) {
    store_le32(col1_token + 4, kLimitExceeded);
    return;
  }
}
