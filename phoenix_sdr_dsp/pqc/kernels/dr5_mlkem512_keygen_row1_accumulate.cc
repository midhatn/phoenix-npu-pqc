...
void dr5_mlkem512_keygen_row1_accumulate(
    const uint8_t * __restrict mat_in,       // 3152 bytes (MatrixToken)
    uint8_t * __restrict final_out           // 2144 bytes (FinalToken)
) {
    // Copy secret prefix (header + rho + z + s0 + s1 + t0)
    for (uint32_t i = 0; i < mlkem512_dr5::kStateE1Offset; ++i) {
        final_out[i] = mat_in[i];
    }
    
    auto *hdr = reinterpret_cast<uint32_t *>(final_out);
    if (hdr[2] != 0) return;
    
    const uint8_t *s0_raw = mat_in + mlkem512_dr5::kSecretS0Offset;
    const uint8_t *s1_raw = mat_in + mlkem512_dr5::kSecretS1Offset;
    const uint8_t *e1_raw = mat_in + mlkem512_dr5::kStateE1Offset;
    const uint8_t *a0_raw = mat_in + mlkem512_dr5::kMatrixA0Offset;
    const uint8_t *a1_raw = mat_in + mlkem512_dr5::kMatrixA1Offset;
    
    uint8_t *t1_out = final_out + mlkem512_dr5::kFinalT1Offset;
    ...
    for (uint32_t i = 0; i < 64; ++i) {
        ...
        uint32_t e1_0 = mlkem512_dr5::load_le16(e1_raw + 2 * (4 * i + 0));
        uint32_t e1_1 = mlkem512_dr5::load_le16(e1_raw + 2 * (4 * i + 1));
        
        uint32_t t0 = prod0_0 + prod1_0 + e1_0;
        if (t0 >= mlkem512_dr5::kQ) t0 -= mlkem512_dr5::kQ;
        if (t0 >= mlkem512_dr5::kQ) t0 -= mlkem512_dr5::kQ;
        
        uint32_t t1 = prod0_1 + prod1_1 + e1_1;
        if (t1 >= mlkem512_dr5::kQ) t1 -= mlkem512_dr5::kQ;
        if (t1 >= mlkem512_dr5::kQ) t1 -= mlkem512_dr5::kQ;
        
        mlkem512_dr5::store_pair_word(t1_out, 2 * i + 0, t0, t1);
        ...
        uint32_t e1_2 = mlkem512_dr5::load_le16(e1_raw + 2 * (4 * i + 2));
        uint32_t e1_3 = mlkem512_dr5::load_le16(e1_raw + 2 * (4 * i + 3));
        
        uint32_t t2 = prod0_2 + prod1_2 + e1_2;
        if (t2 >= mlkem512_dr5::kQ) t2 -= mlkem512_dr5::kQ;
        if (t2 >= mlkem512_dr5::kQ) t2 -= mlkem512_dr5::kQ;
        
        uint32_t t3 = prod0_3 + prod1_3 + e1_3;
        if (t3 >= mlkem512_dr5::kQ) t3 -= mlkem512_dr5::kQ;
        if (t3 >= mlkem512_dr5::kQ) t3 -= mlkem512_dr5::kQ;
        
        mlkem512_dr5::store_pair_word(t1_out, 2 * i + 1, t2, t3);
    }
}
