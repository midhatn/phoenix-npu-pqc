...
void dr19_hybrid_session_service(
    ...
    uint32_t magic = (uint32_t)desc[0] | ((uint32_t)desc[1] << 8) | ((uint32_t)desc[2] << 16) | ((uint32_t)desc[3] << 24);
    uint32_t req_id = (uint32_t)desc[4] | ((uint32_t)desc[5] << 8) | ((uint32_t)desc[6] << 16) | ((uint32_t)desc[7] << 24);
    uint32_t epoch = (uint32_t)desc[8] | ((uint32_t)desc[9] << 8) | ((uint32_t)desc[10] << 16) | ((uint32_t)desc[11] << 24);

    uint32_t status = 0;

    if (magic != DR19_DESC_MAGIC) {
        status = 1;
    } else {
        ...
        // Hardware zeroize internal sponge state
        for (int i = 0; i < 200; i++) state[i] = 0;
        ...
        // Zero local key buffer
        for (int i = 0; i < 32; i++) key[i] = 0;
        return;
    }

    // Fail-closed return on error
    for (int i = 0; i < 128; i++) res[i] = 0;
    res[0] = 0x48; res[1] = 0x53; res[2] = 0x31; res[3] = 0x39;
    res[4] = (uint8_t)(req_id & 0xFF);
    res[8] = (uint8_t)(status & 0xFF);
}
