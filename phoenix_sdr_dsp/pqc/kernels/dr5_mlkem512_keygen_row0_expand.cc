...
void dr5_mlkem512_keygen_row0_expand(
    const uint8_t * __restrict sec_in,       // 2128 bytes (SecretToken)
    uint8_t * __restrict mat_out             // 3152 bytes (MatrixToken)
) {
    // Copy secret token directly into matrix token base
    for (uint32_t i = 0; i < mlkem512_dr5::kSecretTokenBytes; ++i) {
        mat_out[i] = sec_in[i];
    }
    ...
    const uint8_t *rho = sec_in + mlkem512_dr5::kRhoOffset;
    
    // Sample A[0, 0] and A[0, 1] (SHAKE128(rho || j || i) where row=0, col=0 and 1)
    if (!sample_ntt(rho, 0, 0, mat_out + mlkem512_dr5::kMatrixA0Offset) ||
        !sample_ntt(rho, 1, 0, mat_out + mlkem512_dr5::kMatrixA1Offset)) {
