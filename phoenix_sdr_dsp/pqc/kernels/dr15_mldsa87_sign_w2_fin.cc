// SPDX-License-Identifier: Apache-2.0
// DR15 ML-DSA-87 Sign Worker 2 (Encode, Seal & Hardware CRC32)
#include "dr15_mldsa87_internal.hpp"

using namespace phoenix_sdr_dsp::pqc::dr11;
using namespace phoenix_sdr_dsp::pqc::dr12;
using namespace phoenix_sdr_dsp::pqc::dr13;
using namespace phoenix_sdr_dsp::pqc::dr14;
using namespace phoenix_sdr_dsp::pqc::dr15;

extern "C" void dr15_mldsa87_sign_w2_fin(
    const uint8_t in_token[9300],
    uint8_t result[4656]) {

  clear_bytes(result, 4656);

  const uint32_t request_id = load_le32(in_token + 0);
  const uint8_t *c_tilde = in_token + 4;
  const int32_t *z_plain = reinterpret_cast<const int32_t *>(in_token + 68);
  const uint8_t *h_plain = in_token + 7236;

  uint8_t *sig = result + 20;

  // 1. Pack c_tilde (64 B)
  DR11_DISABLE_UNROLL
  for (uint32_t i = 0; i < 64; ++i) sig[0 + i] = c_tilde[i];

  // 2. Pack z (7 * 640 = 4480 B)
  for (uint32_t j = 0; j < 7; ++j) {
    encode_z_poly65(z_plain + j * 256, sig + 64 + j * 640);
  }

  // 3. Pack hints (83 B)
  uint8_t h_arr[8][256];
  for (uint32_t i = 0; i < 8; ++i) {
    DR11_DISABLE_UNROLL
    for (uint32_t c = 0; c < 256; ++c) h_arr[i][c] = h_plain[i * 256 + c];
  }
  encode_hints87(h_arr, sig + 4544);

  // 4. Header + CRC32
  store_le32(result + 0, 0x4434524Du); // b"MR4D"
  store_le32(result + 4, request_id);
  store_le32(result + 8, 0);           // Status OK
  store_le32(result + 12, 4627);       // Signature length
  store_le32(result + 16, 0);          // Placeholder

  const uint32_t crc = compute_crc32(sig, 4627);
  store_le32(result + 16, crc);

  clear_bytes(h_arr, sizeof(h_arr));
}
