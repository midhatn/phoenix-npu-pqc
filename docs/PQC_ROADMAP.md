...
| DR2c | FROZEN / EXISTING | One terminal ML-KEM-512 `K-PKE.KeyGen` \(\widehat{t}\) row. | Physically validated DR2 constituent. Local commit `74c735b`; the premature remote branch was deleted and no pull request exists. It does not close DR2. |
| DR2 | OPEN / REQUIRED FOR CLOSURE | Complete device-resident ML-KEM-512 `K-PKE.KeyGen`. | Requires every condition in Section 4. |

The existing design and physical records remain authoritative for the details
of DR0, DR1, DR2a, DR2b, and DR2c. This roadmap records their place in the
...
FROZEN: DR1
    |
    v
FROZEN: DR2a -> DR2b -> DR2c
    |
    v
PROPOSED: DR2d
Complete K-PKE.KeyGen and close DR2
    |
    v
PROPOSED: DR3
...
## 4. DR2 completion contract

### 4.1 DR2 closure statement

DR2 closes only when a complete device-resident ML-KEM-512
`K-PKE.KeyGen` graph is physically validated. DR2a, DR2b, and DR2c are frozen
constituent baselines; neither their individual validation nor commit
`74c735b` establishes DR2 closure.

### 4.2 DR2d proposed boundary

| Contract area | Required DR2 closure condition |
|---|---|
| Designation | `DR2d` is the proposed label for the increment that closes DR2. |
| Public ingress | `d[32]` and a descriptor, using exactly two host fills. If packing is revised, the accepted design must still have exactly two fills and no widened public boundary. |
| Device derivation | Compute `G(d || k)` on device, where `k = 2` for ML-KEM-512. Neither `rho` nor `sigma` may be derived by the host after dispatch. |
| Matrix generation | Generate all four \(\widehat{A}[i,j]\) matrix polynomials for both ML-KEM-512 rows on device. |
| Noise generation | Generate and transform \(\widehat{s}[0]\), \(\widehat{s}[1]\), \(\widehat{e}[0]\), and \(\widehat{e}[1]\) on device with the required counters. |
| Core computation | Compute both terminal \(\widehat{t}\) rows using the complete matrix-vector accumulation. DR2c's one-row graph is a validated building block, not the terminal DR2 graph. |
| Terminal serialization | Encode and assemble `ekPKE` (800 B) and `dkPKE` (768 B) into one terminal record. |
| Host visibility | The packed terminal key record is the only result eligible for CPU transfer after dispatch. |
| Private intermediates | Keep seeds derived by `G`, noise material, secret polynomials, matrix entries, products, accumulators, and serialization scratch in internal FIFOs or tile-local memory. |
| Lifecycle behavior | Define and test initialization, normal completion cleanup, error cleanup, request reuse, and all execution-model reset boundaries. Secret and private scratch must be explicitly cleared before reuse or release. |
| Failure behavior | Fail closed on invalid public input, descriptor error, bounded sampling failure, token corruption, DMA failure, native build/load failure, incomplete terminal output, or device execution failure. No reference fallback or partial key output is allowed. |
| Validation corpus | Pass all 25 applicable ML-KEM KeyGen ACVP cases, with vector identity and exact result mapping retained. |
| Physical evidence | Retain repeated native runs, placement, worker section sizes, stack allocation, FIFO placement/depth/banks, DMA routes, shim allocations, xclbin/PDI identity, and terminal corpus evidence. |
| Governance | Complete the DR2 closure record, obtain explicit user acceptance, and only then push complete DR2 immediately before DR3 begins. |

### 4.3 DR2 non-closure conditions

DR2 remains open if any of these conditions is true:

1. Only one \(\widehat{t}\) row is terminally validated.
2. `G(d || k)`, a matrix row, a required noise polynomial, an accumulation, or
   key serialization is performed or completed on the CPU.
3. Execution requires an intermediate CPU drain, host-mediated continuation,
   or reference fallback.
4. `ekPKE` and `dkPKE` are not emitted in one terminal record.
5. The public graph does not preserve exactly two host fills.
6. Zeroization and error semantics are undocumented or untested.
7. Any applicable one of the 25 KeyGen ACVP cases is missing, failing, or not
   traceable to retained evidence.
8. Physical build, placement, routing, memory, artifact, and repeated-execution
   evidence is incomplete.
9. The closure record lacks explicit user acceptance.
...
- **Input boundary:** A packed request carries `ek` (800 B) and a deterministic
  32-byte randomness/test input. A second ingress carries the descriptor.
- **Output boundary:** One terminal record carries `c` (768 B) and `K` (32 B).
- **Residency:** Required `H`, `G`, KDF, and accepted device `K-PKE.Encrypt`
  computation remain on device.
- **Randomness boundary:** The explicit 32-byte value provides deterministic
  validation. It does not define or approve a production entropy architecture.
- **Prohibited paths:** No CPU hash/KDF completion, intermediate drain, or
  reference fallback.
- **Acceptance:** All applicable vendored ACVP encapsulation cases,
...
| Review date | 2026-08-17, Asia/Riyadh |
| Overall roadmap decision | **APPROVED** |
| Approved milestones | DR2d boundary and DR3 through DR15 sequence |
...
| Separate approvals | None |
| Notes and constraints | DR2 remains open. No incomplete DR is pushed. Push completed DR2 immediately before DR3 begins. |
| Acceptance reference | Explicit user approval recorded in this Computer thread on 2026-08-17 at 23:03 Asia/Riyadh |

The roadmap boundaries are accepted. DR2 remains **OPEN / REQUIRED FOR
CLOSURE**. DR3 through DR15 remain **ACCEPTED ROADMAP BOUNDARY / NOT STARTED**
and are governed by the sequential closure gates.
