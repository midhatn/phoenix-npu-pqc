# Post-Quantum Cryptography on AMD Phoenix NPU: Publication Readiness & Artifact Evaluation

**Document Version**: 2.0.0 (Baseline Scope)
**Date**: August 29, 2026
...
<!-- [CLAIM-PROVENANCE: status=HISTORICAL; source=publication_readiness_v1; classification=SELF_REPORTED_UNVERIFIED] -->
**Historical Baseline Scope**: 19 / 19 GATES PASS — 736 / 736 TEST CASES EVALUATED
...
<!-- [CLAIM-PROVENANCE: status=HISTORICAL; source=publication_readiness_v1; classification=SELF_REPORTED_UNVERIFIED] -->
This repository contains 100% device-resident research artifacts evaluating implementation of the finalized **NIST Post-Quantum Cryptography Standards (FIPS 202, FIPS 203, FIPS 204)** on the AMD Phoenix NPU (AIE2 / XDNA1 Architecture).

### Core Research Focus
1. **Device Residency & Data Movement**:
   Cryptographic transformations—including SHA-3/SHAKE sponge absorption/squeezing, Keccak-f[1600] permutations, Montgomery arithmetic, Number Theoretic Transforms (NTT/INTT), Centered Binomial Noise Sampling (CBD), rejection sampling loops, Hint bit-manipulation, and sealed CRC32 checksums—target execution on physical AIE2 tiles.
2. **Coverage Across Standard Security Categories**:
   - **NIST FIPS 202**: SHA3-224, SHA3-256, SHA3-384, SHA3-512, SHAKE128, SHAKE256 (Streaming absorb/squeeze).
   - **NIST FIPS 203 (ML-KEM / Kyber)**: Security Categories 1, 3, 5 (ML-KEM-512, ML-KEM-768, ML-KEM-1024) across `KeyGen`, `Encaps`, and `Decaps` with implicit rejection.
...
## 2. Complete Silicon Evaluation Scope (Historical Baseline Scope)

> [!NOTE]
> <!-- [CLAIM-PROVENANCE: status=HISTORICAL; source=publication_readiness_matrix; classification=SELF_REPORTED_UNVERIFIED] -->
> [HISTORICAL CLAIM - UNVERIFIED / PENDING PHYSICAL DISPATCH CORROBORATION]
> Legacy test totals below represent pre-refactor self-reported metrics. Under current Phase A zero-speculation policy, 19 gates are actively tracked with 0 independently physically verified gates pending driver-level dispatch trace corroboration.

| Gate | Milestone | Algorithm & Operation | Silicon Verification Script | Test Count | Baseline Scope | Runtime |
|---|---|---|---|---|---|---|
| **0** | DR0 | M33 Ring Product Vector Unit | `test_m33_product_dr0.py` | 24 | 24 cases tracked | 0.93s |
| **1** | DR1 | ML-DSA-44 ExpandA / RejNTT | `test_dr1_mldsa44_rejntt_silicon.py` | 33 | 33 cases tracked | 0.74s |
| **2** | DR2a | ML-KEM-512 SampleNTT Stream | `test_dr2a_mlkem512_samplentt_silicon.py` | 13 | 13 cases tracked | 0.67s |
| **3** | DR2b | ML-KEM-512 CBD3/NTT Noise | `test_dr2b_mlkem512_noise_ntt_silicon.py` | 13 | 13 cases tracked | 0.72s |
| **4** | DR2c | ML-KEM-512 KeyGen Matrix Row | `test_dr2c_mlkem512_keygen_row_silicon.py` | 11 | 11 cases tracked | 0.71s |
| **5** | DR2d | ML-KEM-512 K-PKE.KeyGen Pipeline | `test_dr2d_mlkem512_kpke_keygen_silicon.py` | 25 | 25 cases tracked | 0.77s |
| **6** | DR3 | ML-KEM-512 K-PKE.Encrypt Pipeline | `test_dr3_mlkem512_kpke_encrypt_silicon.py` | 25 | 25 cases tracked | 0.72s |
| **7** | DR4 | ML-KEM-512 K-PKE.Decrypt Pipeline | `test_dr4_mlkem512_kpke_decrypt_silicon.py` | 25 | 25 cases tracked | 0.73s |
| **8** | DR5 | ML-KEM-512 ML-KEM.KeyGen Graph | `test_dr5_mlkem512_keygen_silicon.py` | 25 | 25 cases tracked | 0.75s |
| **9** | DR6 | ML-KEM-512 ML-KEM.Encaps Graph | `test_dr6_mlkem512_encaps_silicon.py` | 25 | 25 cases tracked | 0.74s |
| **10** | DR7 | ML-KEM-512 ML-KEM.Decaps Graph | `test_dr7_mlkem512_decaps_silicon.py` | 25 | 25 cases tracked | 0.80s |
| **11** | DR8 | ML-KEM-768 & 1024 Expansion | `test_dr8_mlkem_unified_silicon.py` | 75 | 75 cases tracked | 1.80s |
| **12** | DR9 | NIST FIPS 202 SHA-3/SHAKE Service | `test_dr9_fips202_silicon.py` | 122 | 122 cases tracked | 0.86s |
| **13** | DR10 | Sealed Lifecycle & Key Sources | `test_dr10_sealed_lifecycle_silicon.py` | 40 | 40 cases tracked | 1.14s |
| **14** | DR11 | NIST FIPS 204 ML-DSA-44 KeyGen | `test_dr11_mldsa44_keygen_silicon.py` | 25 | 25 cases tracked | 0.94s |
| **15** | DR12 | NIST FIPS 204 ML-DSA-44 Sign | `test_dr12_mldsa44_sign_silicon.py` | 30 | 30 cases tracked | 2.29s |
...
| **TOTAL**| **DR0-15** | **Universal NIST PQC Suite** | `test_all_silicon_gates.py` | **736** | **736 cases tracked** | **23.82s** |
