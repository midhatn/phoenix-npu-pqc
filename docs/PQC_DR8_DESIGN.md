...
1. **KeyGen (5 Workers)**:
   - Worker 0: $G(d \parallel 3) 	o (ho, \sigma)$, sample $s_0..s_2$ ($	ext{CBD}_2$), $e_0..e_2$ ($	ext{CBD}_2$).
...
3. **Decaps (6 Workers)**:
   - Worker 0: $	ext{K-PKE.Decrypt}(dk_{PKE}, c) 	o m'$, forward $H(ek), z, ho, c, \mathbf{t}$.
...
1. **KeyGen (6 Workers)**:
   - Worker 0: $G(d \parallel 4) 	o (ho, \sigma)$, sample $s_0..s_3$ ($	ext{CBD}_2$), $e_0..e_3$ ($	ext{CBD}_2$).
