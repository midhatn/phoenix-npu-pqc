...
| M32e | ML-KEM.KeyGen / Encaps / Decaps composer | ✅ | ML-KEM-512 only: 60 host KATs plus 9 hardware smoke vectors (3 each for key generation, encapsulation, and decapsulation). The host composes calls to M32b/c/d through a `SiliconBackend` seam. Design: [docs/M32e_DESIGN.md](M32e_DESIGN.md). |
