// Purpose: Streaming Single-Pass Multi-Stage SDR Demodulator Pipeline (Mixer -> FIR -> Power).
// Target operating system: Windows 11 Pro 25H2.
// Target architecture: AMD Ryzen 9 7940HS Phoenix / XDNA1 / AIE2.
// Architecture constraint: Zero stack-allocated large arrays (AIE2 stack limit is 32KB).
// Processing mode: Streaming block-by-block with 8-sample history register.
...
void sdr_pipeline_kernel(
    ...
    // Small register shift window for I and Q FIR history (only 8 floats each)
    float hist_i[8] = {0.0f};
    float hist_q[8] = {0.0f};

    // Streaming single-pass: Downconvert -> Shift FIR -> Compute Power
