...
// M33a - Dilithium NTT / INTT / basemul kernel for FIPS 204 ML-DSA (Post-Quantum
// Cryptography).
//
// Ring: Z_q[X]/(X^256 + 1), q = 8380417 (23-bit prime).
// Primitive 512-th root of unity: zeta = 1753.
// Coefficient representation: signed int32 in centered form (approx (-q, q)).
//
// This kernel is a straight-line transliteration of the pq-crystals ML-DSA
// reference C implementation (ref/ntt.c, ref/invntt.c, ref/reduce.c). The
// Montgomery reduction uses Q = 8380417 and QINV = 58728449 such that
// Q * QINV = 1 mod 2^32. Zetas are pre-scaled by R = 2^32 mod Q and stored as
// signed int32 (bit-reversed order, index 0 hard-coded to 0 per ref-C).
//
// Modes (via u8 mode input, matching the M32 family convention):
//   MODE_NTT     = 0  : forward NTT, in-place on 256 int32 coeffs
//   MODE_INTT    = 1  : inverse NTT, in-place; output multiplied by 41978
//                       (= mont^2 / n mod q, signed) so that a chained
//                       ntt(x) -> intt(y) round-trip returns x * R mod q.
//   MODE_BASEMUL = 2  : pointwise Montgomery product of two int32 arrays.
//                       Result semantics: (a * b * R^-1) mod q, signed.
//   MODE_REDUCE  = 3  : coefficient-wise Barrett/centered reduce of an int32
//                       array to representative in (-q, q).
//
// The composer bridges Montgomery <-> plain modular arithmetic in host Python
// exactly as M32b's SiliconBackend does for Kyber (see docs/M32e_DESIGN.md,
// section "NTT convention and the Montgomery bridge").
//
// References
//   FIPS 204, Module-Lattice-Based Digital Signature Standard, NIST, 13 Aug
//     2024, https://nvlpubs.nist.gov/nistpubs/fips/nist.fips.204.pdf
...
// Dispatch entry (host-callable, matches M32b signature style).
void dilithium_ntt(uint8_t mode,
                   int32_t in_a[MAX_COEFFS],
                   int32_t in_b[MAX_COEFFS],
                   int32_t out_c[MAX_COEFFS]) {
    // For NTT/INTT/REDUCE the operand lives in in_a; caller copies to out_c
    // when dispatching, or the host may swap buffers. For BASEMUL, uses both.
    switch (mode) {
        case 0: {  // NTT
            for (int32_t i = 0; i < N; ++i) out_c[i] = in_a[i];
            ntt_kernel(out_c);
            break;
        }
        case 1: {  // INTT
        ...
        case 2: {  // BASEMUL
            basemul_kernel(out_c, in_a, in_b);
            break;
        }
        case 3: {  // REDUCE
            for (int32_t i = 0; i < N; ++i) out_c[i] = in_a[i];
