...
def main() -> int:
    ...
    try:
        ...
        print(f"Backend: dr7-mlkem512-decaps:unavailable ({type(exc).__name__}: {exc})")
    ...
    for idx, case in enumerate(PRE_SILICON_CORPUS):
        ...
        try:
            actual_k = graph.run_mlkem512_decaps(case.dk, case.c, request_id=case.request_id)
        except Exception as exc:
        ...
        if actual_k == expected_k:
            ...
            print(f"  {case.label:<32} FAIL: mismatch")
            case_results.append({
                "case_id": case_id,
                "status": "FAIL",
                "duration_ns": t_case_duration,
