...
"""Host unit tests and NIST ACVP reference validation for DR15 (ML-DSA-87 KeyGen, Sign, Verify)."""
...
def _load_keygen_corpus() -> tuple[DR15KeyGenCase, ...]:
    f = DATA_DIR / "dr15_nist_acvp_mldsa87_keygen_25.json"
    vectors = json.loads(f.read_text(encoding="utf-8"))
    return tuple(
        DR15KeyGenCase(
            tc_id=v["tcId"],
            test_name=f"acvp_mldsa87_keygen_tc{v['tcId']:03d}",
            seed=bytes.fromhex(v["seed"]),
            expected_pk=bytes.fromhex(v["expected_pk"]),
            expected_sk=bytes.fromhex(v["expected_sk"]),
            request_id=0xD1510000 + idx + 1,
        )
        for idx, v in enumerate(vectors)
    )


def _load_sign_corpus() -> tuple[DR15SignCase, ...]:
    f = DATA_DIR / "dr15_nist_acvp_mldsa87_sign_30.json"
    vectors = json.loads(f.read_text(encoding="utf-8"))
    return tuple(
        DR15SignCase(
            tc_id=v["tcId"],
            test_name=f"acvp_mldsa87_sign_tc{v['tcId']:03d}",
            ...
            request_id=0xD1520000 + idx + 1,
        )
        for idx, v in enumerate(vectors)
    )


def _load_verify_corpus() -> tuple[DR15VerifyCase, ...]:
    f = DATA_DIR / "dr15_nist_acvp_mldsa87_verify_30.json"
    vectors = json.loads(f.read_text(encoding="utf-8"))
    return tuple(
        DR15VerifyCase(
            tc_id=v["tcId"],
            test_name=f"acvp_mldsa87_verify_tc{v['tcId']:03d}",
...
TOTAL_DR15_CASES = len(KEYGEN_CORPUS) + len(SIGN_CORPUS) + len(VERIFY_CORPUS)
assert TOTAL_DR15_CASES == 85


class DR15ReferenceTests(unittest.TestCase):
    def test_dr15_case_counts(self) -> None:
        self.assertEqual(len(KEYGEN_CORPUS), 25)
        self.assertEqual(len(SIGN_CORPUS), 30)
        self.assertEqual(len(VERIFY_CORPUS), 30)
        self.assertEqual(TOTAL_DR15_CASES, 85)

    def test_dr15_vector_structures(self) -> None:
        for case in KEYGEN_CORPUS:
            with self.subTest(kg_case=case.test_name):
                self.assertEqual(len(case.seed), 32)
                self.assertEqual(len(case.expected_pk), 2592)
                self.assertEqual(len(case.expected_sk), 4896)

        for case in SIGN_CORPUS:
            with self.subTest(sign_case=case.test_name):
                self.assertEqual(len(case.sk), 4896)
                self.assertEqual(len(case.expected_sig), 4627)

        for case in VERIFY_CORPUS:
            with self.subTest(ver_case=case.test_name):
                self.assertEqual(len(case.pk), 2592)
                self.assertEqual(len(case.sig), 4627)
